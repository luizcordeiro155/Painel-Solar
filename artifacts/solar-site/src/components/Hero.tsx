...
import { ArrowRight, Droplets, ShieldCheck, Wrench, Star, Sparkles, SunMedium } from "lucide-react";
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[100dvh] flex items-center pt-28 pb-14 overflow-hidden bg-slate-950">
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_28%,rgba(245,158,11,0.34),transparent_28rem),linear-gradient(100deg,rgba(2,6,23,0.98)_0%,rgba(15,23,42,0.88)_45%,rgba(15,23,42,0.50)_100%)]" />
        <div className="absolute inset-x-0 bottom-0 h-44 bg-gradient-to-t from-background to-transparent" />
      </div>

      <div className="solar-glow right-[-7rem] top-24 z-[1]" />
      <motion.div
        aria-hidden="true"
        animate={{ rotate: 360 }}
        transition={{ duration: 36, repeat: Infinity, ease: "linear" }}
        className="absolute right-[8%] top-[20%] z-[1] hidden h-52 w-52 rounded-full border border-primary/20 md:block"
      />
        ...
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] items-center gap-12">
          <div className="max-w-4xl">
            ...
            <motion.h1
              ...
              className="max-w-5xl text-5xl font-black leading-[0.95] tracking-[-0.055em] text-white md:text-7xl lg:text-8xl"
            >
          ...
          >
            <div className="premium-card p-6 text-slate-900">
              <div className="relative z-10 rounded-[1.5rem] bg-slate-950 p-6 text-white shadow-2xl">
                <div className="mb-8 flex items-center justify-between">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.24em] text-primary">Projeto solar</p>
                    <h3 className="mt-2 text-2xl font-black">Economia inteligente</h3>
                  </div>
                  <motion.div
                    animate={{ rotate: [0, 12, -8, 0], scale: [1, 1.08, 1] }}
                    transition={{ duration: 4.4, repeat: Infinity }}
                    className="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary text-white shadow-lg shadow-primary/40"
                  >
                    <SunMedium size={28} />
                  </motion.div>
                </div>
                ...
                <div className="space-y-4">
                  {["Análise personalizada", "Instalação profissional", "Suporte humanizado"].map((item, index) => (
                    <motion.div
                      key={item}
                      initial={{ opacity: 0, x: 18 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: 0.72 + index * 0.14 }}
                ...
                <div className="mt-8 grid grid-cols-2 gap-4">
                  <div className="rounded-2xl bg-white p-4 text-slate-950">
                    ...
                    <p className="text-xs font-bold text-slate-500">pré-atendimento</p>
                  </div>
                  <div className="rounded-2xl bg-primary p-4 text-white">
                    <p className="text-3xl font-black">100%</p>
                    <p className="text-xs font-bold text-white/80">sob medida</p>
                  </div>
                </div>
              </div>
            </div>
